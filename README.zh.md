...
把你的 **ChatGPT(Codex)**、**Claude**、**Grok(X Premium)**、**Gemini(Google 账号)** 订阅当作 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 的 LLM provider 使用 —— 不需要 API key。Codex、Grok 和 Gemini 通过 dsh web 界面 OAuth 登录(设置 → 订阅);Claude 在存在 Claude Code 会话时直接导入凭据(macOS Keychain 或 `~/.claude/.credentials.json`),否则回退到同样的浏览器 OAuth 流程,因此不要求安装 Claude Code CLI。Token 保存在 `~/.dsh/plugins/subscriptions/auth.json`(权限 0600),过期自动刷新。

## 演示

设置 → **订阅**:每个 provider 的登录/退出,无需 API key。Claude 有 Claude Code 会话时导入凭据,否则和 Codex、Grok、Gemini 一样走 OAuth(截图中账号已打码):
...
1. `dsh web`,打开打印的 URL。
2. **设置 → 订阅**:点对应 provider 的「连接」。若先运行过 `claude` 并登录,Claude 会即时导入凭据;没有凭据时,Claude 也和其他 provider 一样在浏览器里授权。Codex、Grok 和 Gemini 在打开的标签页里授权;无浏览器环境下可展开手动兜底,粘贴回调 URL 或授权码。首次登录 Gemini 时还会为账号开通托管的 Code Assist 项目,开通完成前卡片一直显示连接中,通常几秒,开通较慢时最多两分钟。注意 Gemini 登录申请的是 Google 的 `cloud-platform` scope(与 Gemini CLI 一致,也是 Code Assist 的要求):存下来的 token 权限范围大于 Gemini 本身,同样保存在权限 0600 的存储里。
...
- `src/translate/` —— dsh `Message[]` 与 OpenAI Responses / Anthropic Messages / Gemini generateContent 格式互转,SSE → `StreamChunk`
